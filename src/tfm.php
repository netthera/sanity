...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if (empty($f['file']['error']) && !empty($tmp_name) && $tmp_name != 'none' && $isFileAllowed) {
    		//nicholas
    		$lock_path=$path . '/' . $f['file']['name'];
    		if (file_exists($path . '/' . $f['file']['name'])){
    			if(snt_islocked($lock_path)== $_SERVER['REMOTE_USER'])
    			;//can upload
    			else if(snt_islocked($lock_path)== "")
    			;//can upload
    			else
    			exit();
    		}
...
//file lock
if (isset($_GET['lock'])) {
	$lock_path=FM_PATH.'/'.$_GET['lock'];
	snt_lock_file($lock_path);
	fm_set_msg('File locked', 'ok');
	fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH).'&view='.$_GET['lock']);
}
//file unlock
if (isset($_GET['unlock'])) {
	$lock_path=FM_PATH.'/'.$_GET['unlock'];
	snt_unlock_file($lock_path);
	fm_set_msg('File unlocked', 'ok');
	fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH).'&view='.$_GET['unlock']);

}
//file compare
if (isset($_GET['comprevs'])) {
	
	fm_show_header(); // HEADER
  fm_show_nav_path(FM_PATH); // current path
	include 'src/revs.php';
	fm_show_footer();
    exit;
}
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
           ...
           //nicholas
           			$lock_path=FM_PATH.'/'.urlencode($file);
           			if(snt_islocked($lock_path)!='') 
           					echo 'File locked by: '.snt_islocked($lock_path).'<br>';
           		
								$rev = get_cur_rev($lock_path);
								if ($rev!=''){
                		echo "<a href=?p=".urlencode(trim(FM_PATH))."&oldfile=$rev&file=$lock_path&comprevs=$file>Compare</a>  with latest archived. ";
             				$temp=$rev;
              			$temp_filename = substr(strrchr($temp, "/"), 1);
              			$temp=substr($rev,0,strrpos ($temp, "/"));
               			echo "<a href=?p=$temp&view=$temp_filename>View</a> latest archive. ";
								}
            }
            ?>
        </p>
        <p>
            ...
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;unlock=<?php echo urlencode($file) ?>"><i class="fa fa-unlock"></i> Unlock</a></b> &nbsp;
            <b><a target="_blank"  href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace" class="edit-file"><i class="fa fa-pencil-square"></i> Edit</a></b> &nbsp;

            <?php }else{ ?>
            <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;lock=<?php echo urlencode($file) ?>"><i class="fa fa-lock"></i> Lock</a> </b> &nbsp;
...
<td><div class="filename"><a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?>
...
<tr>
<?php if (!FM_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td><?php endif; ?>
<td><div class="filename"><a href="<?php echo $filelink ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?></a>
	<?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?>
	<?php //nicholas
	
           			$lock_path=FM_PATH.'/'.urlencode($f);
           			if(snt_islocked($lock_path)!='') 
           					echo '&nbsp;&nbsp;<span class=gray><i class="fa fa-lock"></i> '.snt_islocked($lock_path).'</span>';
           					?>
	</div></td>
...
<a href="https://github.com/netthera/sanity" target="_blank" class="float-right" style="color:silver">Sanity ODE</a></p>
<?php endif; ?>
</form>
...
<div class="path main-nav">
        ...
        <?php if (!FM_READONLY): ?>
        <form style=" display: inline;" ><input type=hidden name=p value=<?php echo urlencode(FM_PATH); ?>> <input size=37 title="Grep searches inside all .php and .js files" placeholder="Enter a word or a phrase(case sensitive)" type=text name=pattern value=''><input title="Grep searches inside all .php and .js files" type=submit value='Grep'></form>&nbsp;&nbsp;&nbsp;&nbsp;

        <a title="Find files or folders" href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="fa fa-search"></i></a>
...
<div id="wrapper">
  <div id="createNewItem" class="modalDialog"><div class="model-wrapper"><a href="#close" title="Close" class="close">X</a><h2>Create New Item</h2><p>
    ...
    <div id="searchResult" class="modalDialog"><div class="model-wrapper">
    	<a href="#close" title="Close" class="close">X</a>
    Find inside .php and .js files
    <?php //nicholas ?>
    <hr>
    Find files and folders
    <input type="search" name="search" value="" placeholder="Find an item in current folder...">
